...
export function baseHonoMiddlewares(middlewares: MiddlewareHandler[], target: any, key?: string | symbol) {
  if (key) {
    // method decorator
    metadata.merge(
      [target.constructor, sController],
      {
        handlers: {
          [key]: {
            middlewares
...
export function baseHonoHandlerDecorator(method: StandartMethod | string, path: string = '') {
